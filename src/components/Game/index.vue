<template>
    <div class="m-puzzle-wrapper">
        <div class="start-game" v-show="isStart">
            <div>
                <button @click="beginGame()">开始游戏</button>
            </div>
        </div>
        <div class="puzzle-wrapper" v-show="isShow"></div>
...
export default {
    ...
    methods: {
         initial(){
            this.isStart = true
            this.isShow = false
            const $puzzle = document.querySelector('.start-game');
            $puzzle.style['background'] = `url("${this.image}")`;
            $puzzle.style['background-size'] = "100% 100%";   
            $puzzle.style['transition'] = `all 2s`;
        ...
        beginGame(){
            ...
            let length = 1;
            let inner = '';
            let piece = Math.pow(this.rowBlock,2)
            while (length <= piece) {
        ...
        bindEvent() {
            ...
            this.picWidth = this.width / this.rowBlock;
            this.picHeight = this.height / this.rowBlock;
        ...
        bindPuzzleImage() {
            ...
            for (let i = 0; i < this.picEl.length; i++) {
                ...
                this.picEl[i].addEventListener('touchend', function (e) {
                    this.style.zIndex = 0;
                    this.style.transition = 'all 0.2s ease 0s'; //添加css3动画效果
        ...
        isSuccess() {
            ...
            if (this.enableTransationEnd && str === numStr) {
                this.start = false;
                this.enableTransationEnd = false;
                this.Success = true;
                return true;
        ...
        setImage() {
            ...
            this.picEl.forEach(item => {
                ...
                const topN = Math.floor((_index - 1) / this.rowBlock);
                const leftN = (_index - 1) % this.rowBlock;
        ...
        randomPuzzleImage() {
            ...
            for (let i = 0; i < 20; i++) { //随机打乱
                let a = Math.floor(Math.random() * piece);
                let b = Math.floor(Math.random() * piece);
                if (a != b) {
